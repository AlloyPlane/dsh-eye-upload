...
> 📸 截图/GIF 待补充 —— 下面预留演示位，实际使用时点 📷 → 选图 → 路径自动填入 → 发送 → 代理看图。

| 步骤 | 说明 |
|------|------|
| **① 点 📷** | 输入框工具行左侧出现相机按钮 |
| **② 选图** | 选择任意 PNG/JPG/WebP/GIF（≤10MB） |
| **③ 自动填入** | 输入框自动出现 `请用 image_understand 查看这张图片：<路径>` |
| **④ 发送** | 回车，代理调用眼睛看图并描述 |

<!-- 演示 GIF 占位：把真实录屏放到 docs/demo.gif 后替换此行 -->
![演示](docs/demo.gif)

<!-- 截图占位：多张界面截图放 docs/ 目录后引用 -->
| | |
|---|---|
| ![界面 1](docs/screenshot-1.png) | ![界面 2](docs/screenshot-2.png) |
